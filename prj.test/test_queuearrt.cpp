...
TEST_CASE("queuearrt ctor") {
    QueueArrT<float> qf1;
    CHECK_EQ(true, qf1.IsEmpty());
    REQUIRE(qf1.Count() == 0);
    ...
    qi3.Push(i3);
    qi3.Clear();
    REQUIRE(qi3.IsEmpty() == true);
    REQUIRE(qi3.Count() == 0);

    QueueArrT<int> qi4(qi1);
    ...
    QueueArrT<std::string> qs1;
    CHECK_EQ(true, qs1.IsEmpty());
    REQUIRE(qs1.Count() == 0);
    std::string s1 = "cat", s2 = "ball", s3 = "human";
    qs1.Push(s1);
